...
void return_double(double time, double type, double value, int handle)
{
	...
	gml_ds_map_add_double(map, const_cast<char*>("type"),type);
	gml_ds_map_add_double(map, const_cast<char*>("value"), value);
	gml_ds_map_add_double(map, const_cast<char*>("handle"), handle);
...
GMEXPORT double thread_create(double time, double type, double value)
{
	int index;
	thread_key.lock();
...
GMEXPORT double thread_kill(double index)
{
	thread_key.lock();
...
GMEXPORT double thread_free(double index)
{
	thread_key.lock();
